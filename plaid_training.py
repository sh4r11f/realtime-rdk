...
def update_plots(history):
    ...
    # Session title
    _plot_fig.suptitle(f"Task: Plaid, Subject: {sub_id}, Session: {ses_id}, Date: {date.today().strftime('%Y-%m-%d')}", fontsize=16)
    ...
    if valid.sum() == 0:
        ...
        axs[0, 2].set_xlabel('Spatial Frequency, cycles/dva')
        axs[0, 2].set_ylabel('Saccade Y (≈ Up - Down), dva')
        axs[0, 2].set_title('Saccade offset by Spatial Frequency and Temporal Frequency')
    ...
    if labels:
        axs[2, 0].legend(title="temporal Frequency", loc="upper right")
    ...
    # mib_df['saccade_y_signed'] = np.where(mib_df['direction'] == 60, mib_df['saccade_y_dva'], -mib_df['saccade_y_dva'])
    # psycho_df = mib_df.groupby('spatial_frequency').agg(
    #     n_trials=('saccade_y_signed', 'count'),
    #     mean_mib=('saccade_y_signed', 'mean'),
    #     sem_mib=('saccade_y_signed', 'sem')
    # ).reset_index()
    # # only plot if we have at least 2 spatial frequency levels with data
    # if psycho_df.shape[0] < 2:
    #     axs[2, 2].text(0.5, 0.5, "Not enough data", ha='center', va='center')
    #     axs[2, 2].set_title("MIB by Spatial Frequency (correct trials)")
    #     axs[2, 2].set_xlabel("Spatial Frequency (cycles/dva)")
    ...
    #     x_vals = psycho_df['spatial_frequency'].values
    ...
    #     # Fit a Weibull function if we have at least 3 unique spatial frequency levels
    #     if psycho_df['spatial_frequency'].nunique() >= 3:
...
def run_trial(condition, trial_index, stim_params, task_params, history):
    ...
    """
    # Gabor display parameters (position relative to center)
    gabor_params = stim_params["Gabor"]
    gabor_ecc = deg2pix(gabor_params["eccentricity"], mon)
    gabor_pos_jitter = deg2pix(gabor_params["position_jitter"], mon)
    gabor_side = 1 if gabor_params["side"] == "right" else 0
    jitter = np.random.uniform(-gabor_pos_jitter, gabor_pos_jitter)
    if gabor_side == 1:
        gabor_x = gabor_ecc + jitter
    else:
        gabor_x = -gabor_ecc + jitter
    gabor_y = jitter
    # Spatial frequency: GratingStim with units='pix' expects cycles/pixel.
    # Convert cycles/deg -> cycles/pixel by dividing by pixels-per-degree.
    ppd = deg2pix(1.0, mon)
    gabor_sf = condition["spatial_frequency"] / ppd
    added_df = 0.25 / ppd
    # Temporal frequency in cycles/second. We'll advance phase using time-based dt.
    gabor_tf_hz = condition["temporal_frequency"]
    gabor_size_px = deg2pix(gabor_params["size"], mon)

    # Two gabor stimuli for Plaid
    gabor_1 = visual.GratingStim(
        win,
        tex=gabor_params["texture"],
        mask='gauss',
        size=(gabor_size_px, gabor_size_px),
        sf=gabor_sf,  # cycles per pixel
        ori=gabor_params["orientation"],  # degrees
        pos=(gabor_x, gabor_y),
        units='pix',
        contrast=gabor_params["contrast"],
        opacity=gabor_params.get("opacity", 0.5),
        autoLog=False,
    )
    gabor_2 = visual.GratingStim(
        win,
        tex=gabor_params["texture"],
        mask='gauss',
        size=(gabor_size_px, gabor_size_px),
        sf=gabor_sf + added_df,  # cycles per pixel
        ori=-gabor_params["orientation"],  # degrees
        pos=(gabor_x, gabor_y),
        units='pix',
        contrast=gabor_params["contrast"],
    ...
    sac_tol_px = deg2pix(gabor_params["size"], mon) / 2.0 + deg2pix(task_params["saccade_tolerance"], mon)
    fix_tol_px = deg2pix(task_params["fixation_tolerance"], mon)
    hit_region = (
        int(scn_width/2.0 + gabor_x - sac_tol_px),
        int(scn_height/2.0 - gabor_y - sac_tol_px),
        int(scn_width/2.0 + gabor_x + sac_tol_px),
        int(scn_height/2.0 - gabor_y + sac_tol_px)
    )
    ...
    # Step 2: Present fixation + plaid (time-based phase updates)
    ...
    last_phase_update_t = fix_start_t
    while time.time() - fix_start_t < fix_dur and fixated:
        ...
        # Draw fixation + plaid with time-based phase increment
        fix_cross.draw()
        now_t = time.time()
        dt = now_t - last_phase_update_t
        last_phase_update_t = now_t
        dphase = (gabor_tf_hz * dt) % 1.0  # cycles
        if condition["direction"] == 90:
            gabor_1.phase = (gabor_1.phase - dphase) % 1.0
            gabor_2.phase = (gabor_2.phase + dphase) % 1.0
        else:
            gabor_1.phase = (gabor_1.phase + dphase) % 1.0
            gabor_2.phase = (gabor_2.phase - dphase) % 1.0
        gabor_1.draw()
        ...
        if simulation_mode:
            print(time.time() - fix_start_t)
            if time.time() - fix_start_t > 1:
                # in simulation mode, just wait 0.5 seconds and assume fixation acquired
                fixated = True
                break
            else:
    ...
    while time.time() - start_t < sac_init_dur and not got_sac:
        ...
        # Draw plaid with time-based phase increment
        now_t = time.time()
        dt = now_t - last_phase_update_t
        last_phase_update_t = now_t
        dphase = (gabor_tf_hz * dt) % 1.0
        if condition["direction"] == 90:
            gabor_1.phase = (gabor_1.phase - dphase) % 1.0
            gabor_2.phase = (gabor_2.phase + dphase) % 1.0
        else:
    ...
    while time.time() - start_t < sac_dur and not landed_in_target:
        ...
        if simulation_mode:
            if time.time() - start_t > 0.1:
                ...
                trial_data["saccade_x"] = gabor_x + np.random.normal(-sac_tol_px/2, sac_tol_px/2)
                trial_data["saccade_y"] = gabor_y + np.random.normal(-sac_tol_px/2, sac_tol_px/2)
                trial_data["correct"] = 1
                break

        try:
            sample = el_tracker.getNewestSample()
            # Poll next data event but avoid heavy per-frame printing, which
            # can throttle frame rate and make drift appear to change speed.
            _ = el_tracker.getNextData()
        except Exception:
            sample = None
        ...
        if (gx is not None) and (gy is not None):
            gaze_center_x = gx - gabor_cx
            gaze_center_y = -gy + gabor_cy
        else:
            gaze_center_x = gaze_center_y = None
...
for sf in task_params["gabor_spatial_frequencies"]:
    for tf in task_params["gabor_temporal_frequencies"]:
        for direction in task_params["gabor_directions"]:
            all_trials.append({
