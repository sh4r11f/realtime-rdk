...
class EyeLinkCoreGraphicsPsychoPy(pylink.EyeLinkCustomDisplay):
    def __init__(self, tracker, win):
        ...
        # _pictureTarget: single image path (backwards compat)
        # _pictureTargets: list of image paths, one per calibration point
        self._pictureTarget = None
        self._pictureTargets = None
        # Index to keep track of which picture to show for the next target
        self._pictureIndex = 0
        ...
        # optional reward callback to be invoked on calibration acceptance
        self._reward_callback = None
    ...
    def set_reward_callback(self, cb):
        """Set a callable to be invoked when calibration target accepted.

        cb should be a callable taking no arguments.
        """
        try:
            if not callable(cb):
    ...
    def setPictureTarget(self, picture_target):
        ...
        if isinstance(picture_target, (list, tuple)):
            # store list of picture paths
            self._pictureTargets = list(picture_target)
            self._pictureTarget = None
        else:
            # single picture path (backwards compatibility)
    ...
    def update_cal_target(self):
        ...
        if self._calTarget == 'picture':
            # allow either a single picture or a list of pictures
            if (self._pictureTarget is None) and (not self._pictureTargets):
                print('ERROR: Provide a picture (or list of pictures) as the calibration target')
                core.quit()
                sys.exit()

            if self._pictureTargets:
                # validate files exist and create ImageStim for each
                imgs = []
                for p in self._pictureTargets:
                    if not os.path.exists(p):
                        print("ERROR: Picture %s not found" % p)
                        self._display.close()
                        core.quit()
                        sys.exit()
                    imgs.append(visual.ImageStim(self._display, image=p))
                self._calibTar = imgs
            else:
                # single picture
                if os.path.exists(self._pictureTarget):
                    self._calibTar = visual.ImageStim(self._display,
                                                      image=self._pictureTarget)
                else:
                    print("ERROR: Picture %s not found" % self._pictureTarget)
                    self._display.close()
                    core.quit()
        elif self._calTarget == 'spiral':
            thetas = np.arange(0, 1440, 10)
            N = len(thetas)
            radii = np.linspace(0, 1.0, N)*self._targetSize
            x, y = pol2cart(theta=thetas, radius=radii)
            xys = np.array([x, y]).transpose()
    ...
    def setup_cal_display(self):
        ...
        # reset picture index when starting calibration/validation
        self._pictureIndex = 0
    ...
    def draw_cal_target(self, x, y):
        ...
        #         # Pick a random image
        #         self._pictureIndex = np.random.randint(0, len(self._calibTar))
        #         self._calibTar = self._calibTar[self._pictureIndex]
        ...
        if self._calTarget in ['spiral', 'movie']:
            ...
            self._animatedTarget = True
            # If multiple images were provided, pick the next one
            if isinstance(self._calibTar, list):
                if len(self._calibTar) == 0:
                    return
                # advance index so next calibration point shows the next image
                self._pictureIndex += 1
                imgstim = self._calibTar[self._pictureIndex % len(self._calibTar)]
                # set size and pos for this target
                try:
                    imgstim.size = (self._targetSize, self._targetSize)
                except Exception:
                    pass
                imgstim.pos = (xVis, yVis)
                imgstim.draw()
                # give reward
                self.give_reward()
                print("Gave reward for calibration at draw")
            else:
                # single image
                self._calibTar.pos = (xVis, yVis)
                try:
                    self._calibTar.size = (self._targetSize, self._targetSize)
                except Exception:
                    pass
                self._calibTar.draw()
            self._display.flip()
    ...
    # def give_reward(self, reward_voltage=5.0, duration_ms=200, pulses=2, inter_pulse_ms=100):
    #     """
    ...
    def play_beep(self, beepid):
        ...
        # try:
        #     self.give_reward()
        #     print("giving reward for calibration")
        ...
        if DISABLE_AUDIO:
            ...
            if self._calTarget == 'movie':
                ...
                if beepid in [pylink.CAL_TARG_BEEP, pylink.DC_TARG_BEEP]:
                    ...
                    # call reward callback if provided (e.g., give reward on target accept)
                    try:
                        # if self._reward_callback is not None:
                            # self._reward_callback()
                        self.give_reward(pulses=2)
                        print("Calibration reward given")
                    except Exception:
                        pass
    ...
    def get_input_key(self):
        ...
        if self._animatedTarget:
            if self._calTarget == 'spiral':
                ...
                # Make the calibration target bigger and smaller (pulsate)
                scale = 1.2 + 0.2 * sin(core.getTime()*2*pi)
                self._calibTar[self._pictureIndex % len(self._calibTar)].size = (100 * scale, 100 * scale)
                self._calibTar[self._pictureIndex % len(self._calibTar)].draw()
            self._display.flip()
...
def main():
    ...
    calib_images = list(res_dir.glob("monkey_*.png"))
    # pick 5 random pics
    calib_images = list(np.random.choice(calib_images, 5))
    # pass the list of 5 image paths so each calibration point can show a different image
    genv.setPictureTarget([str(p) for p in calib_images])
